...
export default function KecReportsPage() {
  const [filter, setFilter] = useState<FilterStatus>("all");
  const allReports = REPORTS.filter((r) => {
    if (r.estimatedBudget == null) return false;
    return r.escalations.some((e) => e.from === "kel" && e.to === "kec") ||
           r.currentLevel === "kec";
  });
  const filtered =
    filter === "all"
  ...
  const pendingCount = allReports.filter(
    (r) => r.currentStatus === "pending" && r.currentLevel === "kec"
  ).length;
