...
export default function RWReportDetailPage({
  params,
}: {
  ...
  const [action, setAction] = useState<"" | "approve" | "reject" | "forward">("");
  ...
  if (!report) {
    return (
      <div>
        <PageHeader title="Laporan Tidak Ditemukan" backHref="/rw/reports" />
  ...
  const canAct = report.currentLevel === "rw" && report.currentStatus === "pending";

  function handleSubmitAction() {
    setDone(true);
    setTimeout(() => router.push("/rw/reports"), 2000);
  }

  if (done) {
    const labels = { approve: "disetujui", reject: "ditolak", forward: "diteruskan ke Kelurahan" };
    return (
      <div>
        <PageHeader title="Detail Laporan" backHref="/rw/reports" />
        <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-neutral-border p-5">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            ...
            <span className="text-xs text-neutral-text">dari RT {report.rt}</span>
        ...
        <div className="bg-white rounded-xl border border-neutral-border p-5 mt-4">
          ...
          {!action && (
            <div className="flex gap-3 flex-wrap">
              ...
              <Button size="sm" variant="secondary" onClick={() => setAction("approve")}>
                Setujui
              </Button>
            ...
            <div className="space-y-4">
              <div className="p-3 bg-blue-light rounded-lg border border-blue-border">
                <p className="text-sm text-blue-dark">
                  Laporan akan diteruskan ke <strong>Kelurahan</strong> untuk ditindaklanjuti.
                </p>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-neutral-dark">Catatan untuk Kelurahan</label>
                <textarea
                  rows={3}
              ...
              <div className="flex gap-3">
                <Button size="sm" onClick={handleSubmitAction} disabled={!note}>Kirim ke Kelurahan</Button>
          ...
          {action === "approve" && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button size="sm" onClick={handleSubmitAction}>Setujui Laporan</Button>
                <Button size="sm" variant="ghost" onClick={() => setAction("")}>Batal</Button>
              </div>
            </div>
          )}
