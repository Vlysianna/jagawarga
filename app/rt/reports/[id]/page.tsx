...
export default function RTReportDetailPage({
  params,
}: {
  ...
  const [action, setAction] = useState<"" | "accept" | "reject" | "forward">("");
  ...
  if (!report) {
    return (
      <div>
        <PageHeader title="Laporan Tidak Ditemukan" backHref="/rt/reports" />
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <FileText size={48} className="text-neutral-border mb-4" />
          <p className="text-neutral-text text-sm">Laporan dengan ID &quot;{id}&quot; tidak ditemukan.</p>
        </div>
      </div>
    );
  }

  const isIncoming = report.authorId.startsWith("w-");
  const isPending = report.currentStatus === "pending";

  function handleSubmitAction() {
    setDone(true);
    setTimeout(() => router.push("/rt/reports"), 2000);
  }

  if (done) {
    const actionLabels = { accept: "diterima", reject: "ditolak", forward: "diteruskan ke RW" };
    return (
      <div>
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-neutral-text">
            Laporan telah {actionLabels[action as keyof typeof actionLabels]}.
          </p>
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-neutral-border bg-white p-4 sm:p-5">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            ...
            {isIncoming && (
              <span className="text-xs text-neutral-text">dari {report.authorName}</span>
            )}
            {!isIncoming && (
        ...
        <div className="mt-4 rounded-xl border border-neutral-border bg-white p-4 sm:p-5">
          ...
          {!action && (
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              <Button size="sm" variant="secondary" onClick={() => setAction("accept")}>
                Terima
              </Button>
            ...
            <div className="space-y-4">
              <div className="p-3 bg-blue-light rounded-lg border border-blue-border">
                <p className="text-sm text-blue-dark">
                  <strong>Teruskan ke RW:</strong> Anda akan membuat laporan resmi dengan estimasi
                  anggaran. Laporan ini akan diteruskan ke Ketua RW untuk ditindaklanjuti.
                </p>
              </div>
              <Input
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-neutral-dark">Catatan</label>
                <textarea
                  rows={3}
                  placeholder="Tuliskan catatan untuk RW..."
                  value={note}
